...
def initialize_system():
    ...
    try:
        ...
        # Load precomputed FAISS index (FAST!)
...
def main():
    ...
    # Disclaimer
    st.info("ℹ️ This assistant answers questions based on 5 official ECHS training videos. "
    ...
    with st.expander("📝 Browse questions by topic"):
        tab1, tab2, tab3 = st.tabs(["Registration & Eligibility", "Emergency Care", "Referrals & Reimbursement"])
        ...
        with tab1:
            ...
            with col1:
                if st.button("What is ECHS?", key="q1", use_container_width=True):
                    ...
                    st.rerun()
                if st.button("Who is eligible for ECHS?", key="q2", use_container_width=True):
                    st.session_state.question_input = "Who is eligible for ECHS?"
                    st.rerun()
            
            with col2:
                if st.button("How do I register for ECHS?", key="q3", use_container_width=True):
                    st.session_state.question_input = "How do I register for ECHS?"
                    st.rerun()
                if st.button("What documents are required?", key="q4", use_container_width=True):
                    st.session_state.question_input = "What documents are required for ECHS registration?"
                    st.rerun()
        ...
        with tab2:
            ...
            with col1:
                if st.button("Can I get emergency treatment at AFMS hospital?", key="q5", use_container_width=True):
                    st.session_state.question_input = "Can I get emergency treatment at AFMS hospital?"
                    st.rerun()
                if st.button("What is the procedure for emergency treatment?", key="q6", use_container_width=True):
                    st.session_state.question_input = "What is the procedure for emergency treatment in ECHS?"
                    st.rerun()
            
            with col2:
                if st.button("Can my parents be treated at AFMS in emergency?", key="q7", use_container_width=True):
                    st.session_state.question_input = "In an emergency, can my parents be treated at AFMS hospital on cashless basis?"
                    st.rerun()
                if st.button("What about non-empanelled hospitals?", key="q8", use_container_width=True):
                    st.session_state.question_input = "Can I get treatment in non-empanelled hospitals?"
                    st.rerun()
        ...
        with tab3:
            ...
            with col1:
                if st.button("What is HSR?", key="q9", use_container_width=True):
                    ...
                    st.rerun()
                if st.button("Can Non-MIL Polyclinic refer to AFMS?", key="q10", use_container_width=True):
                    st.session_state.question_input = "Can a Non MIL Polyclinic refer ESM to AFMS hospital?"
                    st.rerun()
            
            with col2:
                if st.button("Can 26-year-old dependent be referred?", key="q11", use_container_width=True):
                    st.session_state.question_input = "Can my dependent who is 26 years old be referred by the polyclinic to AFMS Hospital?"
                    st.rerun()
                if st.button("How do I claim reimbursement?", key="q12", use_container_width=True):
                    st.session_state.question_input = "How do I claim medical reimbursement in ECHS?"
                    st.rerun()
