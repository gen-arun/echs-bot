...
def get_groq_client():
    """Create and return a Groq client. Shows a clear error if not configured."""
    try:
        from groq import Groq
    except ModuleNotFoundError:
        st.error(
            "Python package `groq` is not installed. "
    ...
    if not api_key:
        st.error(
            ...
            "Set it in Streamlit Cloud (Secrets or Environment variables) "
...
def embed_text(text: str) -> np.ndarray:
    ...
    # normalize so cosine similarity works better
